...
interface Props {
  initialValues: CmsSize
  mutate: any
}

export default function CMSSizeFormEdit(props: Props) {
  ...
  const formik = useFormikCustom({
    ...
    onSubmit: async (data, actions) => {
      ...
      try {
        const response = await sizesAPI.updateSize(data)
        if (response.status === 200) {
          setSnackbarAlert({ message: 'Update size successfully', severity: 'success' })
        }
        await router.back()
        mutate()
      } catch (e: any) {
